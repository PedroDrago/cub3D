...
void	walk_left(t_camera *cam, char **map, char **minimap)
{
	...
	if (map[(int)(fut_pos.x)][(int)(fut_pos.y)] == '1')
		return ;
	if (cam->dir.x > 0.2 && is_empty_tile(map[(int)(fut_pos.x
				- 0.2)][curr_y]) && is_empty_tile(map[(int)(fut_pos.x
				+ 0.2)][curr_y]))
		cam->pos.x = fut_pos.x;
	else if (cam->dir.x < 0.2 && is_empty_tile(map[(int)(fut_pos.x
				+ 0.2)][curr_y]) && is_empty_tile(map[(int)(fut_pos.x
				- 0.2)][curr_y]))
		cam->pos.x = fut_pos.x;
	if (cam->dir.y > 0.2 && is_empty_tile(map[curr_x][(int)(fut_pos.y
			+ 0.2)]) && is_empty_tile(map[curr_x][(int)(fut_pos.y - 0.2)]))
		cam->pos.y = fut_pos.y;
	else if (cam->dir.y < 0.2 && is_empty_tile(map[curr_x][(int)(fut_pos.y
			- 0.2)]) && is_empty_tile(map[curr_x][(int)(fut_pos.y + 0.2)]))
		cam->pos.y = fut_pos.y;
	update_map(minimap, fut_pos, curr_x, curr_y);
}
